...
evalDesc env = \case
  ...
  DescTensor d1 d2 -> 
    VDescTensor (evalDesc env d1) (evalDesc env d2)
    
  DescSum n a d    -> 
    VDescSum n (evalTy env a) \v -> evalDesc (v:env) d
    
  DescProd n a d   -> 
    VDescProd n (evalTy env a) \v -> evalDesc (v:env) d
...
convTm l t u = case (t, u) of
  ...
  (VSquareMap d f m, VSquareMap d' f' m') -> 
    convDesc l d d' && convTm l f f' && convTm l m m'
  (VExtMap d f m, VExtMap d' f' m') -> 
    convDesc l d d' && convTm l f f' && convTm l m m'
  (VElim d c m n, VElim d' c' m' n') -> 
    convDesc l d d' && convTm l c c' && convTm l m m' && convTm l n n'
...
convTy l t u = case (t, u) of
  ...
  -- Descriptions
  (VSquare d p m, VSquare d' p' m') -> 
       convDesc l d d' 
    && convTy (l + 1) (p (VVar l)) (p' (VVar l)) 
    && convTm l m m'
...
convDesc l d d' = case (d, d') of
  ...
  (VDescTensor a b, VDescTensor a' b') -> 
    convDesc l a a' && convDesc l b b'
  (VDescSum _ a b, VDescSum _ a' b') -> 
    convTy l a a' && convDesc (l + 1) (b (VVar l)) (b' (VVar l))
  (VDescProd _ a b, VDescProd _ a' b') -> 
    convTy l a a' && convDesc (l + 1) (b (VVar l)) (b' (VVar l))
  _ -> False
